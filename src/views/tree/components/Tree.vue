<template>
  <div class="flex flex-col w-full">
    <div
      class="flex group"
      :class="{ 'ml-6': !children.length && level, 'mb-2 pr-4': !level }"
    ...
    >
      ...
      <router-link
        :alt="`Create child of ${branch.name}`"
        :to="`/l/${id}/${branch.id}/create`"
        class="opacity-0 group-hover:opacity-50 hover:opacity-80"
      >
        <svg
          v-if="!addChild"
          xmlns="http://www.w3.org/2000/svg"
    ...
    <div
      ...
      :class="{ 'ml-8': level, 'mb-2 pr-4': !level }"
      :style="`padding-left: ${16 * level + 1}px`"
      v-if="branch.id === $route.params.branch && createMode"
    >
      <input
        class="input input-sm truncate"
        @keydown.enter="addBranch"
...
export default {
  ...
  setup(props) {
    const { getBranch, getChildren, id, activeBranch, createBranch } =
      useTree();
    const { ledger } = useLedger();
    const branch = shallowRef(null);
    const children = shallowRef(null);
    const isCollapsed = shallowRef(
      Boolean(localStorage.getItem(`treeItem_${props.branchId}`))
    );
